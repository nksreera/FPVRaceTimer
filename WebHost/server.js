...
db.each("SELECT rowid as id  from Race ORDER BY rowid desc limit 1", function(err, row) {
currentRaceId = row.id;
});

// select all users
db.each("SELECT * from User", function(err, row) {
      // console.log(row.id + ": " + row.name + " User User ");
...
app.post('/newrace', function(req, res){
    db.run("INSERT INTO Race VALUES('" + req.query.name + "','',CURRENT_TIMESTAMP)", function(){
        // On Complete
...
app.get('/historyData', function(req, res){
    ...
    db.each("SELECT rowid, name, location, createdDate FROM Race order by rowid DESC", function(err, row) {
        histData.push({id:row.rowid, name:row.name, location:row.location, createdDate:row.createdDate, pilots : []});
            ...
            db.each("select distinct raceId, id from Details", function(err1, row1) {
                ...
                for(var x=0;x<histData.length;x++)
                {
                    ...
                    {
                        // Found it
                        histData[x].pilots.push({id:row1.id,name:users[row1.id].name});
                    }
                }
            }, function(){
                // Final Complete
...
console.log(req.query.id);
    var pilotsRaced = [];
    db.each("SELECT DISTINCT id from Details where raceId=" + req.query.id, function(err, row){
            pilotsRaced.push(row.id);
            }, function(){
                // real completed
